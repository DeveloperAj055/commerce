...
class NCM_Narnoo_Helper {
    ...
    public function ncm_reservation( $data ) {
        ...
        // $country_list = $ncm_settings->ncm_country();

        // $country_id = ( isset($contact['country']) && !empty($contact['country']) ) ? $contact['country'] : array();  

        // $contact['country'] = isset($country_list[$country_id]) ? $country_list[$country_id] : '';
        ...
        // $ncm->ncm_write_log( "url => ".$this->url );

        // $ncm->ncm_write_log( json_encode($this->authen) );
